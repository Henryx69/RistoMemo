...
// ===================== WIZARD =====================
function mostraWizardSeNecessario() {
  if (!localStorage.getItem('wizardVisto')) {
...
function chiudiWizard() {
  ...
  localStorage.setItem('wizardVisto', '1');
  showTab('aggiungi', document.getElementById('tab-btn-aggiungi'));
  document.getElementById('f-nome').focus();
  const btn = document.getElementById('btnSave');
  btn.classList.add('pulse');
  setTimeout(() => btn.classList.remove('pulse'), 4500);
}
function chiudiWizardSolo() {
  document.getElementById('wizardOverlay').classList.add('hidden');
  localStorage.setItem('wizardVisto', '1');
...
function aggiornaStatCollezione() {
  ...
  if (arr.length === 0) {
    document.getElementById('statCollezione').style.display = 'none';
    return;
  }
...
function condividiRistorante(id) {
  ...
  if (navigator.share) {
    navigator.share({ title: r.name, text: testo })
      .catch(() => {});
  } else {
...
function mostraTerminiSeNecessario() {
  if (!localStorage.getItem('terminiAccettati')) {
    ...
    const btn = document.getElementById('btnAccettaTermini');
    const timer = setInterval(() => {
      ...
      if (sec <= 0) {
        ...
        btn.disabled = false;
        btn.style.background = 'linear-gradient(135deg, var(--red), var(--red-dark))';
...
function accettaTermini() {
  localStorage.setItem('terminiAccettati', '1');
  document.getElementById('termsModal').style.display = 'none';
  mostraWizardSeNecessario();
}
...
window.addEventListener('DOMContentLoaded', () => {
 ...
 aggiornaInfoBackup();
  ...
  // Mostra termini o direttamente wizard
